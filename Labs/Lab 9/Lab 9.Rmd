...
Expected deaths = # of deaths implied for a particular region given that region's age structure aand national age-specific mortality rates for lip caancer.
...
```{r, include=FALSE}
data_L9 = list(y = observe.i,
               log_e = log(expect.i),
               N = length(observe.i),
               x = aff.i - mean(aff.i))

model_L91 = stan(data = data_L9, file = "~/Grad/STA2201/Lab 9/lab9_1.stan",
                 )
model_L91
```
...
model_L92 = stan(data = data_L9, file = "~/Grad/STA2201/Lab 9/lab9_2.stan",
                 )
model_L92
```
...
model_L93 = stan(data = data_L9, file = "~/Grad/STA2201/Lab 9/lab9_3.stan",
                 )
model_L93
```
...
#   ggplot(aes(log_smr, median_mod1)) + 
#   geom_point() + 
#   geom_point(aes(y = median_mod2)) + 
#   geom_point(aes(y = median_mod3)) + 
#   geom_smooth(aes(y = median_mod1, ymin = lower_mod1, ymax = upper_mod1, colour = "y1"), fill="blue", alpha=0.2) + 
#   geom_smooth(aes(y = median_mod2, ymin = lower_mod2, ymax = upper_mod2, colour = "y2"), 
#               stat="identity", fill="red", alpha=0.2, method = "lm") + 
#   geom_smooth(aes(y = median_mod3, ymin = lower_mod3, ymax = upper_mod3, colour = "y3"), 
#               stat="identity", fill="green", alpha=0.2) + 
...
#   ggplot(aes(x= aff, y = median_mod1)) + 
#   # geom_smooth( method = lm)
#   geom_point(color = "red") + 
#   geom_smooth( method = lm, color = "red") +
#   geom_point(aes(y = median_mod2), color = "green")  + 
#   geom_smooth(aes(x = aff, y = median_mod2), method = lm, color = "green") +
#   geom_point(aes(y = median_mod3), color =  "blue") + 
#   geom_smooth(aes(x = aff, y = median_mod3), method = lm, color = "blue")
  # geom_smooth(aes(median_mod1 ~ aff), method = "lm")
  # geom_smooth(aes(x = aff, y = median_mod1, ymin = lower_mod1, ymax = upper_mod1, colour = "y1"), fill="red", alpha=0.2, method = lm) + 
  # geom_smooth(aes(x = aff, y = median_mod2, ymin = lower_mod2, ymax = upper_mod2, colour = "y2"), 
  #             stat="identity", fill="green", alpha=0.2, method = lm) + 
  # geom_smooth(aes(x = aff, y = median_mod3, ymin = lower_mod3, ymax = upper_mod3, colour = "y3"), 
  #             stat="identity", fill="blue", alpha=0.2, method = lm) + 
  #   #geom_smooth(aes(y = y2, ymin = y2_lwr, ymax = y2_upr, colour = "y2"), 
  #             #stat="identity", fill="red", alpha=0.2) +
  # ylim(-2, 4)
...
#   geom_smooth( method = lm, color = "red") +
#   geom_point(aes(y = median_mod2), color = "green")  + 
#   geom_smooth(aes(x = log_smr, y = median_mod2), method = lm, color = "green") +
#   geom_point(aes(y = median_mod3), color =  "blue") + 
#   geom_smooth(aes(x = log_smr, y = median_mod3), method = lm, color = "blue")
  # geom_smooth(aes(median_mod1 ~ aff), method = "lm")
  # geom_smooth(aes(x = aff, y = median_mod1, ymin = lower_mod1, ymax = upper_mod1, colour = "y1"), fill="red", alpha=0.2, method = lm) + 
  # geom_smooth(aes(x = aff, y = median_mod2, ymin = lower_mod2, ymax = upper_mod2, colour = "y2"), 
  #             stat="identity", fill="green", alpha=0.2, method = lm) + 
...
# res9 %>% 
#   mutate(log_smr = log(observe.i/expect.i)) %>% 
#   ggplot(aes(log_smr, median_mod2)) + 
#   geom_point() 
# 
# res9 %>% 
#   mutate(log_smr = log(observe.i/expect.i)) %>% 
#   ggplot(aes(log_smr, median_mod3)) + 
#   geom_point() 
...
res9 %>% 
  ...
  labs(title = "distriibution of Estimated Theta_i's across Models", x = "Log theta", y = "Count") +
